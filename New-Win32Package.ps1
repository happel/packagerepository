...
<#
    ...
    .EXAMPLE
    $params = @{
        Path        = "c:\dev\packagefactory\packages"
        Application = "AdobeAcrobatReaderDCMUI"
        Type        = "App"
        WorkingPath = "c:\dev\packagefactory\output"
        Import      = $true
    }
...
param (
    ...
    [System.String] $Path = "c:\dev\packagefactory\packages",
        ...
        if (Test-Path -Path $ManifestFile -PathType "Leaf") {
            ...
            # Create the package and import the application
                ...
                Write-Information -MessageData "$($PSStyle.Foreground.Cyan)Importing application: '$ApplicationName'"
                    ...
                    # Remove old application folders
                ...
                if ([System.String]::IsNullOrEmpty($Manifest.Application.Filter)) {
                    ...
                    if ($Manifest.Application.PrePackageCmd.Length -gt 0) {
                        ...
                        $packageSourcePath = $([System.IO.Path]::Combine($SourcePath, "$($ApplicationName)_$($EvergreenApp.Version)"))
                        Write-Information -MessageData "$($PSStyle.Foreground.Cyan)Save to: '$packageSourcePath'"
                        #$Result = $EvergreenApp | Save-EvergreenApp -LiteralPath $packageSourcePath
                        #Copy-Item -Path $SourcePath -Destination $OutputPath
                        Write-Information -MessageData "$($PSStyle.Foreground.BrightGreen)Save to: '$SourcePath'"
                        Write-Information -MessageData "$($PSStyle.Foreground.BrightGreen)Save to: '$outputPath'"
                        Write-Information -MessageData "$($PSStyle.Foreground.BrightGreen)Save to: '$packageSourcePath'"

                        $packagesourcePath = "$WorkingPath\$ApplicationName\Source"
                        $packagepath = $([System.IO.Path]::Combine($WorkingPath, "$($ApplicationName)_$($EvergreenApp.Version)"))
                        ...
                        $TempPath = "$WorkingPath\$ApplicationName"
                        Write-Information -MessageData "$($PSStyle.Foreground.BrightGreen)Delete: '$TempPath'" 
